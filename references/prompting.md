...
## Prompt fields versus CLI flags
- Treat `Use case`, `Asset type`, `Scene/backdrop`, `Subject`, `Style/medium`, `Composition/framing`, `Lighting/mood`, `Color palette`, `Materials/textures`, `Text (verbatim)`, `Constraints`, and `Avoid` as prompt scaffolding only.
- Map those fields into prompt text or the documented CLI options in `references/cli.md`; do not invent new flags from them.
- `Asset type` describes where the image will be used, not a command-line argument.
