---
name: "imagegen6"
description: "Generate or edit raster images when the task benefits from AI-created bitmap visuals such as photos, illustrations, textures, sprites, mockups, or transparent-background cutouts. Use the bundled Responses-first CLI/API workflow by default when Codex should create a brand-new image, transform an existing image, derive visual variants from references, or save project assets. The CLI calls the Responses API hosted image_generation tool when no explicit image size is requested, uses the Images API for exact image controls or edit endpoints, and resolves credentials/base URL from Codex auth/config files. Use the built-in image_gen tool only for inline preview, credential-free fallback, or explicit built-in requests. Do not use when the task is better handled by editing SVG/vector/code-native assets or building visuals directly in HTML/CSS/canvas."
---
...
This skill uses CLI/API by default and has these execution branches:

- **Default Responses-first CLI/API path:** `scripts/image_gen.py`. Use this for normal project-bound generation, edits, image references, exact output paths, batch work, and reproducible API behavior.
- **Responses image_generation branch:** `scripts/image_gen.py generate` with `--api auto` or `--api responses`. Use this when generating without an explicit image size. It calls `POST /v1/responses` with `{"type":"image_generation","output_format":"png"}` in `tools`.
- **Images API branch:** `scripts/image_gen.py` using `/v1/images/generations` or `/v1/images/edits`. Use this when exact size, quality, background, masks, edit inputs, or batch generation are required.
- **Built-in tool fallback branch:** built-in `image_gen` tool. Use only for inline preview, when no usable CLI/API credential exists and the user accepts fallback behavior, or when the user explicitly asks for the built-in tool.
...
- If the user explicitly asks for a transparent image/background, stay on the CLI/API path first: prompt for a flat removable chroma-key background, generate with `scripts/image_gen.py`, then remove it locally with the installed helper at `${CODEX_HOME:-$HOME/.codex}/skills/imagegen6/scripts/remove_chroma_key.py`.
- Never silently switch from the default Responses-first path or CLI `gpt-image-2` to CLI `gpt-image-1.5`. Treat this as a model/path downgrade and ask the user before doing it, unless the user has already explicitly requested `gpt-image-1.5` or true/native transparent output.
- If a transparent request appears too complex for clean chroma-key removal, asks for true/native transparency, or local removal fails validation, explain that true transparency requires Images API `gpt-image-1.5 --background transparent --output-format png` because Responses hosted `image_generation` and `gpt-image-2` do not support native transparent background, then ask whether to proceed. Run that native-transparency path only after the user confirms.
- The word `batch` means CLI/API mode when the user wants saved deliverables or reproducible generation. Use `generate-batch` for many different prompts; use `--n` only for variants of one prompt.
...
Built-in fallback save-path policy:
- In built-in tool mode, Codex saves generated images under `$CODEX_HOME/*` by default.
- Do not describe or rely on OS temp as the built-in fallback destination.
...
Built-in fallback edit semantics:
...
- If a local file needs direct file-path control, masks, or other explicit parameters, use the CLI/API path.
- For edits, preserve invariants aggressively and save non-destructively by default.

Execution strategy:
- In the default CLI/API path, use `generate` for one asset, `generate-batch` for many distinct prompts/assets, and `--n` only for variants of one prompt when the selected API path supports it.
- In built-in fallback mode, produce many assets or variants by issuing one `image_gen` call per requested asset or variant.
- For many distinct assets, do not use `n` as a substitute for separate prompts. `n` is for variants of one prompt; distinct assets need distinct CLI `generate-batch` jobs or distinct built-in fallback calls.

Assume the user wants a new image unless they clearly ask to change an existing one.

## Workflow
1. Decide the execution path: CLI/API by default; use built-in `image_gen` only for inline preview, accepted credential-free fallback, or an explicit built-in request.
2. Decide the intent: `generate` or `edit`.
3. Decide whether the output is preview-only or meant to be consumed by the current project.
4. Decide the execution strategy: single asset vs CLI `generate-batch` vs repeated built-in fallback calls.
...
11. For transparent-output requests, follow the transparent image guidance below: generate through the CLI/API path on a flat chroma-key background, run the installed `${CODEX_HOME:-$HOME/.codex}/skills/imagegen6/scripts/remove_chroma_key.py` helper, and validate the alpha result before using it. If this path looks unsuitable or fails, ask before switching to Images API `gpt-image-1.5`.
12. Inspect outputs and validate: subject, style, composition, text accuracy, and invariants/avoid items.
13. Iterate with a single targeted change, then re-check.
14. For built-in preview-only work, render the image inline; the underlying file may remain at the default `$CODEX_HOME/generated_images/...` path.
...
Transparent-image requests use the CLI/API path first. Because the Responses hosted tool and `gpt-image-2` Images API path do not expose native transparent-background output, create a removable chroma-key source image and then convert the key color to alpha locally.

Default sequence:
1. Use `scripts/image_gen.py generate` to generate the requested subject on a perfectly flat solid chroma-key background. Omit `--size` to keep the default Responses API path, or use `--api images --size ...` only when exact dimensions are required.
...
3. Write the chroma-key source to a workspace path with `--out`, usually under `tmp/imagegen/` or `output/imagegen/`. If using built-in fallback because credentials are unavailable, move or copy the selected source image from `$CODEX_HOME/generated_images/...` first.
...
Do not automatically use Images API `gpt-image-1.5 --background transparent --output-format png` instead of chroma keying. Ask the user first when the user asks for true/native transparency, when local removal fails validation, or when the requested image is complex: hair, fur, feathers, smoke, glass, liquids, translucent materials, reflective objects, soft shadows, realistic product grounding, or subject colors that conflict with all practical key colors.

Use a concise confirmation like:

```text
This likely needs true native transparency. The default CLI/API path uses a chroma-key background plus local removal, but true transparency requires the Images API path with gpt-image-1.5 because Responses hosted image_generation and gpt-image-2 do not support background=transparent. It also requires a local Codex/OpenAI credential. Should I proceed with that native-transparency path?
```
...
- background-extraction - transparent background / clean cutout. Use CLI/API chroma-key generation with local removal first for simple opaque subjects; ask before using Images API true transparency for complex subjects.
...
- `Scene/backdrop` refers to the visual setting. It is not the same as the Images API `background` parameter, which controls output transparency behavior.
- CLI/API execution notes such as `Quality:`, `Input fidelity:`, masks, output format, and output paths belong in the CLI path only. Do not treat them as built-in `image_gen` tool arguments.
...
- For CLI/API controls, see `references/cli.md` and `references/image-api.md` for model, `quality`, `input_fidelity`, masks, output format, and output-path guidance.
- For transparent images, use the CLI/API chroma-key workflow unless the request is complex enough to need true native transparency; ask before switching to Images API `gpt-image-1.5`.
...
## gpt-image-2 guidance for Images API

The Images API CLI path defaults to `gpt-image-2`.

- Use `gpt-image-2` for Images API workflows unless the request needs true model-native transparent output.
- If a transparent request may need native transparency, ask before using `gpt-image-1.5` unless the user already explicitly requested `gpt-image-1.5` or true/native transparent output. Explain that the default CLI/API chroma-key path comes first, but true transparency requires `gpt-image-1.5` because Responses hosted `image_generation` and `gpt-image-2` do not support `background=transparent`.
...
These conventions apply only to CLI/API work. They do not describe built-in `image_gen` output behavior.
...
## Output naming
- For CLI/API work, if the user does not supply a filename, generate a UUID filename such as `<uuid>.png`.
...
- Network approvals / sandbox settings for CLI/API mode: `references/codex-network.md`
...
- `references/cli.md`: default Responses-first CLI/API usage via `scripts/image_gen.py`.
- `references/image-api.md`: API/CLI parameter reference for Responses hosted `image_generation`, Images API generation, and Images API edits.
- `references/codex-network.md`: network/sandbox troubleshooting for CLI/API mode.
- `scripts/image_gen.py`: default CLI implementation. Prefer running it instead of building one-off image API scripts.
- `${CODEX_HOME:-$HOME/.codex}/skills/imagegen6/scripts/remove_chroma_key.py`: local post-processing helper for transparent-image requests.
